...
class Roombooking_Utils
{
    ...
    **/
    ...
    {
        $action = isset($action) ? $action : Roombooking_AuthType::VIEW;
        ...
        if($role=='cat'){
            ...
            XCube_DelegateUtils::call($delegateName, new XCube_Ref($idList), $accessController->dirname(), self::getActor($dirname, Roombooking_AuthType::VIEW), Legacy_Utils::getUid(), $categoryId);
        }
        elseif($role=='group'){
            $delegateName = 'Legacy_Group.'.$accessController->dirname().'.GetGroupIdListByAction';
            XCube_DelegateUtils::call($delegateName, new XCube_Ref($idList), $accessController->dirname(), $dirname, 'page', Roombooking_AuthType::VIEW);
        }
    ...
    **/
     ...
     * 
     * @return  Roombooking_AbstractAccessController
    **/
	public static function getAccessControllerObject(/*** string ***/ $dirname, /*** string ***/ $dataname)
	{
		...
		if(! $module){
			require_once ROOMBOOKING_TRUST_PATH . '/class/NoneAccessController.class.php';
			$accessController = new Roombooking_NoneAccessController($server, $dirname, $dataname);
		...
		switch($role){
		case 'cat':
			require_once ROOMBOOKING_TRUST_PATH . '/class/CatAccessController.class.php';
			$accessController = new Roombooking_CatAccessController($server, $dirname, $dataname);
			break;
		case 'group':
			require_once ROOMBOOKING_TRUST_PATH . '/class/GroupAccessController.class.php';
			$accessController = new Roombooking_GroupAccessController($server, $dirname, $dataname);
			break;
		case 'none':
		default:
			require_once ROOMBOOKING_TRUST_PATH . '/class/NoneAccessController.class.php';
    ...
    **/
    public static function getActor(/*** string ***/ $dirname, /*** string ***/ $action)
    {
        ...
        switch($action){
            case Roombooking_AuthType::VIEW:
                return trim($authType[0]);
                break;
            case Roombooking_AuthType::POST:
                return trim($authType[1]);
                break;
            case Roombooking_AuthType::MANAGE:
